...
- [INTERNAL] Parakeet release builds now use a hash-locked dependency graph in a clean Python 3.12 environment, transcribe generated speech through the packaged executable, and fail closed unless public zip assets are Developer ID signed, notarized, checksummed, and tied to a clean tagged commit (#987).
- [FIX] Local Parakeet server shutdown now terminates active MLX inference immediately instead of waiting for an uncancellable worker thread (#987).
