...
- Extracted issues now carry `severity`, `component`, and a `deduplicationHint` on Windows (`WIN-013` / #998), using the same FNV-1a hash contract as macOS so the same issue yields the same hint on both platforms. `reproductionSteps` and `screenshotAnnotations` remain macOS-only, and the new fields are not yet shown in the Windows issue editor or included in GitHub/Jira export bodies — both tracked as follow-up slices on #998.
