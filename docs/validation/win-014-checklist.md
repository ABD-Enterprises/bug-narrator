...
- `[automation]` The overlay and capture plumbing under emulated DPI: `ScreenshotCaptureGeometryTests.Overlay_UnderEmulatedScale_ReportsTheDraggedRegionInPhysicalPixels` (the overlay itself, DIP → device transform emulated at 1.25 and 1.5) plus `CaptureUnderEmulatedScale_ProducesABitmapOfTheDraggedRegionInPhysicalPixels` (the pure mapping) and `ScreenshotCaptureGeometryTests.OverlaySpansTheFullVirtualScreen_AndARegionAcrossTheBoundaryMapsConsistentlyOnBothSides` (the overlay constructed over an emulated two-monitor virtual screen, drag across the boundary) (#1134, WIN-015). The overlay now converts its DIP drag through `ScreenshotCaptureGeometry.ToPhysical` before capture — it previously passed DIPs straight to `CopyFromScreen`. The human rows above remain the real-hardware proof; native pixels on a secondary at a different scale need per-monitor V2 awareness (#1186, WIN-041).
