...
- `transcript.md` follows the macOS transcript contract (`WIN-011` / #995): matching section names (`## Markers`, `## Screenshots`, `## Raw Transcript`), date/duration formats, fallback text, and LF line endings. The review summary and extracted issues moved out of `transcript.md` into a `summary.md` written beside it and included in the exported session bundle, so the exported bundle is `transcript.md` + optional `summary.md` + `screenshots/`. Screenshot-to-marker linkage is rendered from recorded timeline relationships (#1043). Windows deliberately does not segment transcripts: it exports raw transcript text and timeline markers without a `## Transcript Sections` block (decision recorded in #996 and the parity matrix).
...
- Windows audio-source parity now includes microphone recording, WASAPI loopback system-audio recording, and mixed microphone + system audio recording muxed into a single 16 kHz mono track (`WIN-010` / #453); mixed capture reuses the system-audio consent gate
